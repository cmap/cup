...
# Specify the AWS region (e.g., 'us-west-2', 'us-east-1')
...
if 'email' not in st.session_state or 'init' not in st.session_state or 'api_key' not in st.session_state["init"]:
    # This part simulates a popup by hiding the rest of the app until logged in
    ...
    with placeholder.container():
        ...
        if st.session_state.get("email") and st.session_state.get("init") and st.session_state["init"].get("api_key"):
            # Remove the login placeholder after success
            placeholder.empty()
else:
    # User is logged in, proceed with the main app
    decoded = jwt.decode(st.session_state["init"]["api_key"], jwtTokenSecret, algorithms=["HS256"])
    API_KEY = decoded['api_key']

    if decoded and decoded.get("roles"):
        if 'ADMIN' in decoded["roles"] or 'PRISM_CORE' in decoded["roles"] or 'CMAP_CORE' in decoded["roles"]:
            ...
            st.write("Welcome to the PRISM QC dashboard!")
            ...
            if build:
                ...
                if st.button('Generate report'):
                    with st.spinner('Generating report...'):
                        generate_report.generate_report(build, api_key=API_KEY)
                ...
                if set(expected_plots).issubset(set(existing_plots)):
                    ...
                    with st.spinner('Loading report...'):
                            ...
                            for label, tab in zip(tab_labels, tabs):
                                with tab:
                                    ...
                                    with row2_col1:
                                        ...
                                        io_functions.load_image_from_s3(filename=delta_lmfi_histogram_filename,
                                                                        prefix=build)
                        ...
                        with st.expander('Data removed'):
                            ...
                            tbl_by_plate = io_functions.load_json_table_from_s3(
                                filename='instances_removed_by_plate_table.json', prefix=build)
                        ...
                        with st.expander('Cell line pass/fail'):
                            ...
                            st.table(
                                pass_fail.reset_index(drop=True).style.bar(subset=['Pass'], color='#006600', vmin=0,
                                                                           vmax=100).bar(
